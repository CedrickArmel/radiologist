...
| `eval.yaml` | Evaluation-only variant of `train.yaml`. Same group wiring except `loggers: null` and no `module/scheduler`/`module/optimizer` (not needed without a fit loop). Sets `stage: eval`, `train: false`, `test: true`, `ckpt_path: ???` (mandatory — must be supplied via `ckpt_path=...` on the CLI), and `optimized_metric: test_score`. |
