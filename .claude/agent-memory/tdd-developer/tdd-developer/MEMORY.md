...
- [fastapi/Prefect version cap (#192)](project_fastapi_prefect_cap_192.md) — root `[tool.uv] constraint-dependencies` caps fastapi<0.116.0 to keep Prefect's embedded router working; canary test guards regressions
- [Pinned subagent cannot EnterWorktree](feedback_pinned_subagent_cannot_enterworktree.md) — a worktree-pinned subagent can't hop worktrees via EnterWorktree/ExitWorktree; reuse own pinned dir or branch from there instead
